...
function SteamParticles({ count, reducedMotion }: SteamParticlesProps) {
  ...
  const geometry = useMemo(() => new THREE.PlaneGeometry(0.075, 0.72, 1, 8), []);
  ...
  useFrame(({ clock }) => {
    ...
    groupRef.current.children.forEach((child, index) => {
      ...
      child.position.x = particle.x + Math.sin(elapsed * 0.7 + particle.phase) * 0.055;
      child.position.y = particle.y + Math.sin(elapsed * 0.42 + particle.phase) * 0.035;
      child.rotation.z = Math.sin(elapsed * 0.5 + particle.phase) * 0.22;
    });
  });
